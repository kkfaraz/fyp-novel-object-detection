...
class LVISEvalCustom(LVISEval):
    ...
    def _get_gt_dt(self, img_id, cat_id):
        if self.params.use_cats:
            gt = self._gts.get((img_id, cat_id), [])
            dt = self._dts.get((img_id, cat_id), [])
        else:
            gt = [
                _ann
                for _cat_id in self.params.cat_ids
                for _ann in self._gts.get((img_id, _cat_id), [])
            ]
            dt = [
                _ann
                for _cat_id in self.params.cat_ids
                for _ann in self._dts.get((img_id, _cat_id), [])
            ]
        return gt, dt

    def evaluate(self):
        """
        Optimized version of evaluate to prevent out-of-memory and bus errors,
        and drastically speed up the evaluation process.
        """
        self.logger.info("Running optimized per image evaluation.")
        ...
        # Step 1: Only compute IOUs for active (image_id, cat_id) pairs
        from collections import defaultdict
        self.ious = defaultdict(list)
        
        # self._gts and self._dts only contain keys that have ground truths or detections
        active_keys = set(self._gts.keys()).union(self._dts.keys())
        
        # Filter active_keys to keep only those with img_ids and cat_ids of interest
        img_ids_set = set(self.params.img_ids)
        cat_ids_set = set(cat_ids)
        active_keys = {
            (img_id, cat_id) for img_id, cat_id in active_keys
            if img_id in img_ids_set and cat_id in cat_ids_set
        }

        self.logger.info(f"Computing IOUs for {len(active_keys)} active image-category pairs (out of {len(self.params.img_ids) * len(cat_ids)} total combinations)...")
        for img_id, cat_id in active_keys:
            # We only need IOU if there is both GT and DT, otherwise match cannot occur
            if len(self._gts.get((img_id, cat_id), [])) > 0 and len(self._dts.get((img_id, cat_id), [])) > 0:
                ious = self.compute_iou(img_id, cat_id)
                if len(ious) > 0:
                    self.ious[img_id, cat_id] = ious

        # Step 2: Loop through images, area range, max detection number
        self.logger.info("Evaluating images...")
        self.eval_imgs = []
        for cat_id in cat_ids:
            for area_rng in self.params.area_rng:
                for img_id in self.params.img_ids:
                    if (img_id, cat_id) in self._gts or (img_id, cat_id) in self._dts:
                        res = self.evaluate_img(img_id, cat_id, area_rng)
                        self.eval_imgs.append(res)
                    else:
                        self.eval_imgs.append(None)
