...
from load_models import load_fully_supervised_trained_model, load_clip_model, load_clip_model_caption_based, load_sam_model
...
def setup(outputs_dir, gdino_checkpoint, cfg_file, rcnn_weight_dir, sam_checkpoint, class_len_per_prompt):
    ...
    rcnn_model, cfg = load_fully_supervised_trained_model(cfg_file, rcnn_weight_dir, device=device)

    coco_to_lvis = get_coco_to_lvis_mapping(cfg, lvis_data_split)

    use_caption_based_clip = params.get("use_caption_based_clip", False)
    if use_caption_based_clip:
        clip_model, preprocess, text_features, lvis_classes = load_clip_model_caption_based(lvis_data_split, device)
    else:
